...
// const RANDOM_NAMES = ["DataSense", "Diya6", "Zara", "Kian", "Riya", "Ayaan21", "Mia", "Kabir", "Sana", "Joel3"];
// const PLAYER_POOL = RANDOM_NAMES.map((name, i) => ({ name, avatar: `/players/p${(i % 8) + 1}.png` }));
...
//   const getDifficulty = () => {
//     if (mode === 'bullet_surge') {
//         if (timeMinutes <= 5) return "easy";
//         if (timeMinutes >= 15) return "advanced";
//         return "medium";
//     }
//     return difficulty === 'beginner' ? 'easy' : difficulty === 'intermediate' ? 'medium' : 'advanced';
//   };

//   const todayChallenge = useMemo(() => DAILY_CHALLENGES[new Date().getDate() % DAILY_CHALLENGES.length], []);
//   const onlinePlayers = useMemo(() => [...PLAYER_POOL].sort(() => Math.random() - 0.5).slice(0, 8), []);
...
//     setTilt({ x: (x - 0.5) * -4, y: (y - 0.5) * 4 }); // Reduced intensity slightly to prevent click miss
//   };
...
//   return (
//     // FIX 1: Use 'fixed' background container to isolate from content flow
...
//             initial={{ opacity: 0.4 }} animate={{ opacity: [0.2, 0.5, 0.2] }}
...
//       <audio ref={audioRef} src="/sound/battlesound.mp3" loop />

//       {/* --- CONTENT LAYER (z-10) --- */}
//       {/* FIX 2: Removed 'transformStyle: preserve-3d' to avoid hit-test bugs in some viewports */}
//       <motion.div
//         className="relative z-50 w-full max-w-[1400px] mx-auto flex min-h-screen flex-col px-4 md:px-8 py-4"
...
//               <span className="bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent filter drop-shadow-[0_0_8px_rgba(34,211,238,0.5)]">
//                 SQL Battleground
//               </span>
//             </h1>
//             <p className="text-[11px] text-slate-300/80 font-mono">
//               CONFIGURE MISSION // SQUAD UP // DROP IN
//             </p>
//           </div>
...
//               className="flex h-9 w-9 items-center justify-center rounded-full border border-cyan-400/60 bg-slate-900/80 text-xs shadow-[0_0_12px_rgba(34,211,238,0.5)] hover:bg-slate-800 transition z-20 cursor-pointer pointer-events-auto"
//             >
...
//                 <span className="font-semibold">{user?.fullName || "Agent"} &mdash; Level 9</span>
//               </div>
...
//         {/* MAIN GRID */}
//         <div className="grid flex-1 grid-cols-1 md:grid-cols-[2fr,1fr] gap-4 pb-4">
          
//           {/* LEFT SIDE (Config) */}
//           <div className="flex flex-col gap-3">
            
//             {/* 1. Mode */}
//             <div className="rounded-2xl border border-slate-700/60 bg-slate-900/80 p-4 shadow-lg shadow-black/40 backdrop-blur-sm relative z-20">
//               <div className="mb-2 flex items-center justify-between">
//                 <h2 className="flex items-center gap-2 text-[11px] font-semibold tracking-wide text-cyan-400">
//                   <Sword className="h-4 w-4" /> Choose Your Challenge
//                 </h2>
...
//                 <ModePill label="Daily Challenge" active={mode === "daily_dash"} onClick={() => setMode("daily_dash")} />
//               </div>
...
//             <div className="rounded-2xl border border-slate-700/60 bg-slate-900/80 p-4 shadow-lg shadow-black/40 backdrop-blur-sm relative z-20">
//               <div className="mb-2">
//                 <h2 className="flex items-center gap-2 text-[11px] font-semibold tracking-wide text-emerald-400">
//                   <Target className="h-4 w-4" /> Difficulty
//                 </h2>
//               </div>
//               <div className="mb-4 grid grid-cols-3 rounded-full bg-slate-950 p-1 text-[11px] font-semibold border border-slate-800">
...
//                 <div className="mb-2 flex items-center justify-between">
//                   <h2 className="flex items-center gap-2 text-[11px] font-semibold tracking-wide text-amber-400">
//                     <Flame className="h-4 w-4" /> Focus Areas
//                   </h2>
...
//                 <h3 className="mb-2 flex items-center gap-2 text-[11px] font-semibold tracking-wide text-white">
//                   <Trophy className="h-4 w-4 text-cyan-400" /> Challenge Preview
//                 </h3>
...
//                   </div>
//                 ) : (
//                     <div onClick={() => isSearching && socket?.emit("cancelSearch")} className="cursor-pointer text-slate-400 font-semibold uppercase tracking-wider">
//                         System Idle
//                     </div>
//                 )}
//               </div>
...
//              {/* Level Card */}
//              <div className="rounded-2xl border border-cyan-500/40 bg-gradient-to-r from-slate-900/95 via-slate-900 to-slate-950 p-4 shadow-lg shadow-black/60">
//                 <div className="flex justify-between items-center mb-2">
//                     <div>
//                         <p className="text-[10px] font-semibold text-cyan-300/90">Player Level</p>
//                         <p className="text-sm font-bold">Level 9 &middot; JOIN Table Pro</p>
//                     </div>
...
//                      <div className="h-full w-3/4 bg-gradient-to-r from-cyan-400 via-emerald-400 to-amber-300" />
//                 </div>
//                 <div className="flex justify-between text-[9px] text-slate-400"><span>Total: 1,380</span><span>Next: 420 XP</span></div>
//              </div>

//              {/* Missions */}
//              <div className="flex-1 rounded-2xl border border-slate-700/70 bg-slate-900/85 p-4 shadow-md flex flex-col min-h-[150px]">
//                 <div className="mb-2 flex items-center justify-between">
//                     <h2 className="flex items-center gap-2 text-[11px] font-semibold tracking-wide text-amber-300">
//                         <Medal className="h-4 w-4" /> Missions & Badges
//                     </h2>
...
//                                 <p className="text-[9px] text-slate-500">{b.description}</p>
//                             </div>
//                             <div className="text-right">
//                                 <p className="text-[10px] font-bold text-emerald-400">+{b.xp} XP</p>
//                             </div>
//                         </div>
//                     ))}
//                 </div>
...
//              {/* Player Strip */}
//              <div className="flex items-center gap-3 rounded-2xl border border-slate-700/80 bg-slate-900/90 px-3 py-2 shadow-lg">
...
//                          <div key={i} className="flex flex-col items-center min-w-[30px] opacity-70 hover:opacity-100 transition">
//                              <img src={p.avatar} className="h-6 w-6 rounded-full border border-slate-700 bg-slate-800" alt={p.name} />
//                              <span className="text-[7px] mt-0.5 text-slate-400">{p.name}</span>
//                          </div>
//                      ))}
//                  </div>
//              </div>
...
//                 <p className="mb-1 text-xs font-semibold text-amber-300">{todayChallenge.title}</p>
//                 <p className="mb-2 text-[11px] text-slate-200">{todayChallenge.description}</p>
//                 <p className="text-[11px] font-semibold text-emerald-300">Reward: +{todayChallenge.xp} XP</p>
//               </div>
...
// const ModePill: React.FC<{ label: string; active: boolean; onClick: () => void }> = ({ label, active, onClick }) => (
//   <button onClick={onClick} className={`rounded-full px-3 py-1.5 text-center transition-all cursor-pointer ${active ? "bg-gradient-to-r from-cyan-400 to-emerald-400 text-slate-900 shadow-[0_0_16px_rgba(34,211,238,0.7)]" : "text-slate-300 hover:bg-slate-700/80"}`}>{label}</button>
// );
